...
class CircleChanger(object):
    ...
    def __init__(self, x, y, radius, fill_color, colors):
        ...
        ################################################################
        center = rg.Point(x, y)
        newCirc = rg.Circle(center, radius)
        newCirc.fill_color = fill_color
    ...
    def swell_or_shrink_once(self, amount_to_swell_or_shrink):
        ...
        randex = random.randrange(0, len(self.colors))
        self.circle.fill_color = self.colors[randex]
    ...
    def swell_or_shrink_repeatedly(self, amount_to_swell_or_shrink, times_to_swell_or_shrink):
        ...
        for k in range(times_to_swell_or_shrink - 1):
            ...
            self.draw()
        self.swell_or_shrink_once(amount_to_swell_or_shrink * -k)
    ...
    def swallow(self, other_circle_changer):
        ...
        new_point = self.circle.center.halfway_to(other_circle_changer.circle.center)
        rad = self.circle.center.get_distance_from(new_point)
        new_changer = CircleChanger(new_point.x, new_point.y, rad, 'red', self.colors + other_circle_changer.colors)
        return new_changer
    ...
    def change_to_original_color(self):
        ...
        #   (below).  Third, implement and test this method.
        ################################################################
        self.circle.fill_color = self.ogc
    ...
    ################################################################
        self.count += 1
        self.count = self.count % len(self.colors)
        self.change_color(self.count)
